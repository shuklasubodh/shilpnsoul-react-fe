...
function TrackOrder() {
  ...
  const resend = async (resendChannel) => {
    setLoading(true);setError('');setNotificationMessage('')
    try{await orderApi.resendGuestSummary(result.id,result.order_access_token,resendChannel);const method=resendChannel==='WHATSAPP'?'WhatsApp':resendChannel==='SMS'?'SMS':'email';setNotificationMessage(`Order summary sent by ${method}.`)}
